def temperature_calculation(delta_m_color_index_array):
    ...
    for i in xrange(len(delta_m_color_index_array)):
        temperature_array.append(7920/(delta_m_color_index_array[i] + 0.72))
...
def get_string_with_temperature(delta_m_color_index_array):
    ...
    return {
        "header":"temperature",
        "content":
        "minimum temperature: " + str(temperature_array[0]) + ", "
        + "maximum temperature: "+ str(temperature_array[1]) + ", "
        +  "average_temperature: " + str(temperature_array[2])}